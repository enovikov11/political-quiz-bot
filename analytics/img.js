const sharp = require('sharp'),
    state = require('/Users/enovikov11/Desktop/state.json'), { getUserPoint } = require('../src/logic'),
...
(async () => {
    ...
    await sharp(svg480)
        .png().toFile(`./analytics/out/data-480.png`);

    await sharp(svg1280)
        .png().toFile(`./analytics/out/data-1280.png`);

    await sharp(svg480full)
        .png().toFile(`./analytics/out/data-avatar-480.png`);

    await sharp(svg1280full)
        .png().toFile(`./analytics/out/data-avatar-1280.png`);

    await sharp('./analytics/back.png')
        .composite([
            { input: svg480, blend: 'over', left: 176, top: 164 }
        ]).png().toFile(`./analytics/out/data-border.png`);

    await sharp('./analytics/back.png')
        .composite([
            { input: svg480full, blend: 'over', left: 176, top: 164 }
        ]).png().toFile(`./analytics/out/data-avatar-border.png`);
